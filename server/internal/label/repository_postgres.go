...
func (r *PostgresRepository) Create(ctx context.Context, l Label) (Label, error) {
	...
	const q = `INSERT INTO labels (id, name, color) VALUES ($1, $2, $3)`
...
func (r *PostgresRepository) List(ctx context.Context) ([]Label, error) {
	const q = `SELECT id, name, color FROM labels ORDER BY name ASC`
...
func (r *PostgresRepository) Update(ctx context.Context, labelID string, input UpdateInput) (Label, error) {
	...
	q := fmt.Sprintf(
		"UPDATE labels SET %s WHERE id = $%d RETURNING id, name, color",
...
func (r *PostgresRepository) get(ctx context.Context, labelID string) (Label, error) {
	const q = `SELECT id, name, color FROM labels WHERE id = $1`
...
func (r *PostgresRepository) Delete(ctx context.Context, labelID string) error {
	res, err := r.db.ExecContext(ctx, `DELETE FROM labels WHERE id = $1`, labelID)
	if err != nil {
...
// violation (SQLSTATE 23505) — labels.name is unique, so this is how a
// duplicate name surfaces as a 400 instead of a raw 500.
