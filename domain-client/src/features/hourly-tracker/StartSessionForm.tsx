...
export function StartSessionForm({ onStart, onClose }: StartSessionFormProps) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md max-h-[85vh] overflow-y-auto themed-scrollbar rounded-xl border-(--line) border-[0.5px] border-solid bg-(--card) p-5 shadow-lg"
      ...
      >
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            disabled={submitting}
            aria-label="Close"
            className="shrink-0 text-(--text-faint) hover:text-(--fg) transition-colors cursor-pointer disabled:opacity-50"
          >
            <X size={14} />
          </button>
        </div>
