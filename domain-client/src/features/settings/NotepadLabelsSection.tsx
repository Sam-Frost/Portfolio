...
import { LABEL_COLORS, type LabelColor } from "../labels/types";
import { createNoteLabel, deleteNoteLabel, fetchNoteLabels, updateNoteLabel } from "../notepad/labelApi";
import type { NoteLabel } from "../notepad/types";

// Mirror of settings/LabelsSection, wired to the notepad-label API. The two
// label sets are managed separately (see server/internal/notepadlabel).
...
export function NotepadLabelsSection() {
  ...
  async function handleAdd() {
    ...
    setError(null);
    try {
      const label = await createNoteLabel({ name: trimmed, color: newColor });
  ...
  async function handleSave(id: string, input: { name: string; color: LabelColor }) {
    const updated = await updateNoteLabel(id, input);
