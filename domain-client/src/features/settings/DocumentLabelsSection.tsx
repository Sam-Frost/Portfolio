...
import {
  ...
  fetchDocumentLabels,
  updateDocumentLabel,
} from "../documents/labelApi";
import type { DocumentLabel } from "../documents/types";

// Mirror of settings/LabelsSection, wired to the document-label API. The
// two label sets are managed separately (see server/internal/documentlabel).
...
export function DocumentLabelsSection() {
  ...
  async function handleAdd() {
    ...
    setError(null);
    try {
      const label = await createDocumentLabel({ name: trimmed, color: newColor });
  ...
  async function handleSave(id: string, input: { name: string; color: LabelColor }) {
    const updated = await updateDocumentLabel(id, input);
  ...
  function handleDelete(id: string) {
    ...
    deleteDocumentLabel(id).catch((err) => {
      setLabels(original);
