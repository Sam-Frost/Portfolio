  ...
  return (
    <div className="h-full flex flex-col">
      <div className="shrink-0">
        ...
        <div className="mb-4 flex items-center gap-1 border-b-[0.5px] border-(--line-soft)">
          {TABS.map((t) => (
            <button
              key={t.key}
              type="button"
              onClick={() => setTab(t.key)}
              className={`-mb-px border-b-2 px-3 py-2 text-[length:var(--text-caption)] transition-colors cursor-pointer ${
                tab === t.key
                  ? "border-(--fg) text-(--fg)"
      ...
      <div className="flex-1 min-h-0 overflow-y-auto themed-scrollbar pb-4">
